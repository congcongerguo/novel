...
| s13 | 4-废品站 | 巷子口·废品堆成小山（**纯空镜**·空间建立） | 5.2s | **124** | — | **新**（A1） |
...
| 场 | 镜 | 文件 |
|----|----|------|
| 1 晨·废弃屋 | s01 s02 | `A3_充电头怼三孔圆口.png` `s02_蹲地碾铜脚.png` |
| 2 街头 | s03 s04 s05 | `s03_被当成噪音.png` `s04_让你的模型来问.png` `s05_你没登记吧.png` |
| 3 配给点 | s06–s12 | `s06_配给点长队.png` `s07_核验失败.png` `s08_预约死循环.png` `s09_方案与选择.png` `s10_哪部分经历.png` `s11_把家卖了.png` `s12_街边逆光.png` |
| 4 废品站 | **s13(A1)** s14 s15(A2) s16 s17 s18 s19 s20 | `A1_废品收购站场景.png` `s14_无所事事坐着.png` `A2_锈迹斑斑的旧设备.png` `s16_拨了一下.png` `s17_屏幕光映脸.png` `s18_怎么修好的.png` `s19_塞干粮与硬币.png` `s20_别往新城区说.png` |
| 5 暮·巷口 | s21 | `s21_三种人吃东西.png` |
| 6 夜 | s22 s23 **s24(A4)** s25 | `s22_摸到兜里那个.png` `s23_插上电源口.png` `A4_手机屏幕2%.png` `s25_他抬起头.png` |

> **已定稿的锚点（后续镜一律沿用）**：A1 废品收购站场景（作者确认"效果不错"）· A2 旧设备 · A3 充电头怼三孔圆口 · A4 手机 2%

### 生成口径（出后续集时照此）

1. **带角色正脸／多角色同框** → 传 `image` 参考图（`input/juese/<角色拼音>/<角色>.png`），提示词开头拼 `LY_REF` 段
   - **判断依据是剧本原文**：镜里只要写了陆远的动作（哪怕是"看了看自己的手腕"），他**就必须在画面里**——不要当成"纯环境镜"（s06 踩过：初版出成无人环境图，漏了"陆远看自己空手腕"这个镜眼）
2. **空镜／道具／场景** → 纯文生图
...
7. **⚠️ 锚点必须"被用"，不能只是"被写"（2026-09-12 P0 修复）**
...
10. 脚本：`python tools/seedream_batch.py all`（已存在自动跳过）；改图前先把旧图改名备份（`.v2` / `.preanchor` 等）
...
> **已修正**：s13 改为**纯空镜 124 帧（5.17s）**，只保留环境微动（灰尘/棚布/金属吃光），不再出现任何人物。
> 实测：与 A1 源图的相关度由 `0.993 → 0.776 → 0.506`（旧，尾帧画面大变）变为 `0.999 → 0.900 → 0.851`（新，稳定）。
...
> ⚠️ 另两版归档：`releases/2026-09-12_ep01_成片包/99_历史与废弃/视频_废弃v1_无对白无轴线/`、`..._废弃v2_语法对但未锁首帧/`

**整集粗剪**（无字幕）：`releases/2026-09-12_ep01_成片包/03_镜头视频/_整集粗剪_ep01_无字幕_2026-09-12.mp4`（3:50.49）
**音轨验收件**：`releases/2026-09-12_ep01_成片包/05_质检/音轨验收/*.mp3`（10 个有对白镜，用 `tools/extract_audio.py` 抽）
